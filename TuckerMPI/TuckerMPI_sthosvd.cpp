...
extern float tucker_sv_threshold;

extern "C"
{
  void tuckermpi_sthosvd(float *data, int *I_dims1, int *Grid_dims, char *sthosvd_dir1)
  {
    //
    // Initialize MPI
    //
    ...
    // number_of_layers = number_of_layers1;
    //
    ...
    std::string timing_file = "runtime.csv";
    ...
    if (rank == 0 && boolPrintOptions)
    {
      ...
      std::cout << "- Automatic rank determination = " << (boolAuto ? "true" : "false") << std::endl
                << std::endl;
      ...
      if (!boolAuto)
      {
        ...
        std::cout << "- Ranks = " << *R_dims << std::endl
                  << std::endl;
      }
      ...
      if (boolWriteSTHOSVD)
        std::cout << "NOTE: Please ensure that this directory actually exists!\n";
      ...
      if (boolReconstruct)
        std::cout << "WARNING: This may require a great deal of memory\n";
    ...
    // printf("\n****rank=%d, input tensor size=%d %d %d %d %d****\n", rank, X.getLocalSize()[0], X.getLocalSize()[1], X.getLocalSize()[2], X.getLocalSize()[3], X.getLocalSize()[4]);
    size_t count = X.getLocalNumEntries();
    memcpy(X.getLocalTensor()->data(), data, sizeof(float) * count);
    // X.getLocalTensor()->data_ = data;
    // data = NULL;
    free(data);

    readTimer.stop();

    float localReadTime = readTimer.duration();
    float globalReadTime;

    MPI_Reduce(&localReadTime, &globalReadTime, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
    ...
    if (needToSendToZero)
    {
      ...
      float *mins = Tucker::MemoryManager::safe_new_array<float>(numEntries);
      float *maxs = Tucker::MemoryManager::safe_new_array<float>(numEntries);
      float *means = Tucker::MemoryManager::safe_new_array<float>(numEntries);
      float *vars = Tucker::MemoryManager::safe_new_array<float>(numEntries);
      MPI_Gatherv(metrics->getMinData(), map->getLocalNumEntries(),
                  MPI_FLOAT, mins, (int *)map->getNumElementsPerProc()->data(),
                  (int *)map->getOffsets()->data(), MPI_FLOAT, 0, rowComm);
      MPI_Gatherv(metrics->getMaxData(), map->getLocalNumEntries(),
                  MPI_FLOAT, maxs, (int *)map->getNumElementsPerProc()->data(),
                  (int *)map->getOffsets()->data(), MPI_FLOAT, 0, rowComm);
      MPI_Gatherv(metrics->getMeanData(), map->getLocalNumEntries(),
                  MPI_FLOAT, means, (int *)map->getNumElementsPerProc()->data(),
                  (int *)map->getOffsets()->data(), MPI_FLOAT, 0, rowComm);
      MPI_Gatherv(metrics->getVarianceData(), map->getLocalNumEntries(),
                  MPI_FLOAT, vars, (int *)map->getNumElementsPerProc()->data(),
                  (int *)map->getOffsets()->data(), MPI_FLOAT, 0, rowComm);
    ...
    if (boolSTHOSVD)
    {
      ...
      if (boolReconstruct)
      {
        ...
        float nrm = X.norm2();
        float err = diff->norm2();
        float maxEntry = diff->maxEntry();
        float minEntry = diff->minEntry();
        if (rank == 0)
        {
      ...
      float xnorm = std::sqrt(X.norm2());
      float gnorm = std::sqrt(solution->G->norm2());
      if (rank == 0)
      {
      ...
      if (boolWriteSTHOSVD)
      {
        ...
        float localWriteTime = writeTimer.duration();
        float globalWriteTime;

        MPI_Reduce(&localWriteTime, &globalWriteTime, 1, MPI_FLOAT, MPI_MAX, 0, MPI_COMM_WORLD);
    ...
    if (R_dims)
      Tucker::MemoryManager::safe_delete<Tucker::SizeArray>(R_dims);
