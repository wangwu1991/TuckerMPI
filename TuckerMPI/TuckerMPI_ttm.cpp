              ...
              Tucker::Timer *reduce_timer, size_t nnz_limit)
  {
    ...
    else
    {
      ...
      if (nnz_reduce_scatter <= std::max(max_lcl_nnz_x, nnz_limit))
      {
        ...
        if (reduce_scatter_timer)
          reduce_scatter_timer->start();
        MPI_Reduce_scatter((void *)sendBuf, recvBuf, recvCounts, MPI_FLOAT,
                           MPI_SUM, comm);
        if (reduce_scatter_timer)
          reduce_scatter_timer->stop();
      ...
      else
      {
        for (int root = 0; root < Pn; root++)
        {
          ...
          if (count > 0)
          {
            if (reduce_timer)
              reduce_timer->start();
            MPI_Reduce((void *)sendBuf, recvBuf, (int)count, MPI_FLOAT, MPI_SUM,
                       root, comm);
            if (reduce_timer)
              reduce_timer->stop();
          }
