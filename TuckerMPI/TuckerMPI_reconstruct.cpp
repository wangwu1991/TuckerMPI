...
#include <fstream>
#include "assert.h"
#include "TuckerMPI_reconstruct.hpp"

// int number_of_layers[2];
extern int turcker_number_of_layers[2];

extern "C"
{
  void tuckermpi_reconstruct(float *data, int *Grid_dims, int *sub_end1, char *sthosvd_dir1)
  // int main(int argc, char *argv[])
  {
    ...
    Tucker::SizeArray *subs_begin = Tucker::MemoryManager::safe_new<Tucker::SizeArray>(5);
    Tucker::SizeArray *subs_end = Tucker::MemoryManager::safe_new<Tucker::SizeArray>(5);

    for (int i = 0; i < 5; i++)
    {
      (*subs_begin)[i] = 0;
      (*subs_end)[i] = sub_end1[i];
    ...
    std::string out_fns_file = "rec.txt";
    ...
    // printf("****coreSize = %d %d %d %d %d****\n", (*coreSize)[0], (*coreSize)[1], (*coreSize)[2], (*coreSize)[3], (*coreSize)[4]);
    ...
    // printf("~~~~~size= %d %d %d %d %d~~~~\n", (*I_dims)[0], (*I_dims)[1], (*I_dims)[2], (*I_dims)[3], (*I_dims)[4]);
    ...
    if (rec_order == NULL)
    {
      ...
      float *ratios = Tucker::MemoryManager::safe_new_array<float>(nd);
      for (int i = 0; i < nd; i++)
      {
        rec_size[i] = 1 + (*subs_end)[i] - (*subs_begin)[i];
        ratios[i] = (float)rec_size[i] / (*coreSize)[i];
      }
      ...
      Tucker::MemoryManager::safe_delete_array<float>(ratios, nd);
    }
    ...
    if (scale_mode < nd)
    {
      ...
      float *scales = Tucker::MemoryManager::safe_new_array<float>(scale_size);
      float *shifts = Tucker::MemoryManager::safe_new_array<float>(scale_size);
      if (rank == 0)
      {
      ...
      MPI_Bcast(scales, scale_size, MPI_FLOAT, 0, MPI_COMM_WORLD);
      MPI_Bcast(shifts, scale_size, MPI_FLOAT, 0, MPI_COMM_WORLD);

      int row_begin = result->getDistribution()->getMap(scale_mode, false)->getGlobalIndex(0);
      if (row_begin >= 0)
        TuckerMPI::transformSlices(result, scale_mode, scales + row_begin, shifts + row_begin);

      Tucker::MemoryManager::safe_delete_array<float>(scales, scale_size);
      Tucker::MemoryManager::safe_delete_array<float>(shifts, scale_size);
    ...
    memcpy(data, result->getLocalTensor()->data(), sizeof(float) * result->getLocalNumEntries());
