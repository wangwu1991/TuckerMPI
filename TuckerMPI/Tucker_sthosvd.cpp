...
#include <fstream>
#include "assert.h"
#include "Tucker_sthosvd.h"

extern "C"
{
  void sthosvd_serial(float *data, int *I_dims1, char *sthosvd_dir1)
  {
    ...
    // strcpy(sthosvd_dir, sthosvd_dir1);
    ...
    memcpy(X->data(), data, sizeof(float) * nnz);
    readTimer.stop();
    ...
    // {
    //   float stdev = sqrt(metrics->getVarianceData()[i]);
